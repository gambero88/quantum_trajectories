...
for iz=1:size(z,2) % cicle on the feedback strength
    for ik=1:size(k,2) % cicle on the measurement strength
        for ieta=1:size(eta,2) % cicle on the efficiency
            
            % print status
            istep=istep+1; % update counter
            fprintf('\nProgress... %4.3g%%  \n',100*(istep-1)/(size(z,2)*size(k,2)*size(eta,2)))
            
            % file to be loaded
            str=sprintf('%sDyn%s_k%gfb%geta%g.mat',folder,dyn,k(ik),z(iz),eta(ieta));
            in.file=sprintf('%sDyn%s_k%gfb%geta%g',folder,dyn,k(ik),z(iz),eta(ieta));
