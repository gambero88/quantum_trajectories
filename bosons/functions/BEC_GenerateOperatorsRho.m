...
function [op] = BEC_GenerateOperatorsRho(in);
...
op.mat.abd=sparse(op.abd);

%number operators
op.na(in.N+1,1)=in.N;
op.nb=in.N-op.na;

op.mat.na=sparse(1:D,1:D,op.na);
op.mat.nb=sparse(1:D,1:D,op.nb);

% current operator
op.delta=-sqrt(-1)*(op.abd'-op.abd);
op.delta=sparse(op.delta);

op.mat.delta=op.delta;

%% initialize jump operators
if in.scheme=='diff' % diffraction minimum
    op.ch.jump=op.na-op.nb;
    op.mat.ch.jump=sparse(1:D,1:D,op.ch.jump);
